...
- **Same decision on different machines.** Task `mean`, fixture candidates, probe seed 7, budget 20, executed on the local subprocess backend (`cmp_local`, `v6_local`) and in a ConTree sandbox (`v6_contree`): identical `decision_core_sha256` `b03b4dd4...8b54cf6e`. The demo re-scores the sandbox trace in the browser and gets the same hash; the command to reproduce it locally is under Results. The clarification loop runs on either backend (`loop.py --backend contree`): the fixture session `ctree_e2e` and its local twin `ctree_e2e_local` produced identical hashes in both rounds, and three LLM sessions (`ctree_llm1` to `ctree_llm3`) completed in ConTree sandboxes with zero sandbox errors, 331 to 332 s each.
...
`--backend contree` runs every round in ConTree sandboxes (about 2.7 minutes per round for three candidates, 40 probes, 2 repeats and the mutants, on 8 workers); `--workers` and `--image` pass through to the runner. A session id is used once: `loop.py` refuses to start over an existing session, because re-running would append to the ledger and overwrite `runs/<session>_r1`; `--resume` continues an unfinished session from its next round and is refused for a finished one.

Outputs: `ledger/<session>/ledger.jsonl` (the header records the backend), `clarify/<session>/round_<n>.{task,question_1,verdict}.json`, `runs/<session>_r<n>/`. `python summarize.py` prints a verdict-plus-gold table for every local run. `--research off` skips the Tavily reference search (recorded in the ledger as `status: off`); `python research.py --selftest` checks the query builder and the failure paths without network.
</details>
...
| Same decision across backends | 4 cross-backend groups, 9 runs, 2 backends (local subprocess, ConTree), identical hash within every group: task `mean` (3 runs, `b03b4dd4...`), both rounds of the `percent_change` loop session (`ctree_e2e` vs `ctree_e2e_local`), and round 2 of the demo session `live7` vs `ctree_llm2`, an LLM session that produced the same candidate patches and ran them in a ConTree sandbox | [provenance](https://minjun0208.github.io/behavioral-disagreement-gate/provenance.html) and front page | `python runner.py --task tasks/mean.json --run-id xb --gen hardcoded --backend local --probe-budget 20 && python scorer.py runs/xb/trace.jsonl cfg_full.json`; repeat with `--backend contree`. Loop: `python loop.py --task examples/percent_change_e2e.task.json --session <id> --gen hardcoded --answers examples/percent_change_e2e.answers.json --research off --backend contree`, then the same with `--backend local` and a new session id |
| Answer-key isolation | 49 canary checks, 0 hits | provenance | `python grader.py <run_id> && python canary_check.py <run_id>` |
| Scorer fidelity | stripped trace vs full trace: 100/100 identical decision hashes; JavaScript vs Python: 94/94 in Node and [94/94 in the browser](https://minjun0208.github.io/behavioral-disagreement-gate/test/scorer_test.html) | provenance | `python build_site_data.py --exclude-sessions demo1`; `node site/test/scorer_test.mjs site/data` |
| Live sessions | 7 LLM sessions on `round_half`: 6 `PASS` (five in 2 rounds, one in 3), 1 `UNVERIFIABLE` (`sw_live`: fewer than two distinct implementations left after G4). `live7` is the session in the demo video: witness `x = -12.5`, answer `-13`, one candidate dropped by G4 in round 2. 3 LLM sessions on `round_half` in ConTree sandboxes (`ctree_llm1` to `ctree_llm3`, scripted answer `o2`): 3/3 `PASS` in 2 rounds, 0 sandbox errors. 4 fixture sessions from the ablation, 1 fixture session on ConTree with its local twin. 1 fixture session `ref1` (`safe_div`) with a recorded Tavily reference block, `UNVERIFIABLE` after the answer left one implementation | [front page](https://minjun0208.github.io/behavioral-disagreement-gate/) | `python loop.py --gen llm ...` as above; `python loop.py --task tasks/safe_div.json --session ref1 --gen hardcoded --answers examples/safe_div_ref1.answers.json` |
...
7. **ConTree SDK is pre-alpha** (0.3.6 pinned; `conformance_contree.py` guards the 8 behaviours used). 91 of the 100 committed runs were executed on the local subprocess backend and 9 in ConTree sandboxes; the cross-backend hash identity rests on 4 groups (9 runs).
...
gold/                                    reference tests, grader only; not served by the site; the recorded canary checks found no leak
